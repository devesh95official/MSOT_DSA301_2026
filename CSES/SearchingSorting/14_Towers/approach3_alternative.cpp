...
// The tops, if kept in a vector, are ALWAYS sorted -> binary search on a plain vector
void solve() {
    int n; cin >> n;
    vector<int> tops;                          // sorted ascending at all times
    for (int i = 0; i < n; i++) {
        int k; cin >> k;
        auto it = upper_bound(tops.begin(), tops.end(), k);
        if (it == tops.end()) tops.push_back(k);  // no tower can take it: new tower
        else *it = k;                             // replace that top; order stays sorted
    }
    cout << tops.size() << '\n';
}
