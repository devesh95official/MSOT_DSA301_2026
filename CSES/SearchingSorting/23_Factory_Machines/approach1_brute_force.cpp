...
// simulate: repeatedly let the machine that finishes its next product earliest produce it
void solve() {
    int n, t; cin >> n >> t;
    priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>>> pq;
    for (int i = 0; i < n; i++) { int k; cin >> k; pq.push({k, k}); }
    int last = 0;
    while (t--) { auto [fin, k] = pq.top(); pq.pop(); last = fin; pq.push({fin + k, k}); }
    cout << last << '\n';
}
