...
void solve() {
    int n, x; cin >> n >> x;
    map<int,int> seen;                    // value -> a position where we saw it
    for (int i = 1; i <= n; i++) {
        int v; cin >> v;
        auto it = seen.find(x - v);       // has the partner appeared before?
        if (it != seen.end()) {
            cout << it->second << " " << i << '\n';
            return;
        }
        seen[v] = i;                      // insert AFTER checking -> never pairs i with itself
    }
    cout << "IMPOSSIBLE\n";
}
