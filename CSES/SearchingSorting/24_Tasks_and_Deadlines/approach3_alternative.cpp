...
// Closed form: sum(d) - sum(finish times).
// In sorted order, the i-th shortest duration is "paid" by itself and every later task.
void solve() {
    int n; cin >> n;
    vector<int> a(n);
    int sumD = 0;
    for (int i = 0; i < n; i++) { int d; cin >> a[i] >> d; sumD += d; }
    sort(a.begin(), a.end());
    int sumFinish = 0;
    for (int i = 0; i < n; i++) sumFinish += a[i] * (n - i);   // counted (n - i) times
    cout << sumD - sumFinish << '\n';
}
