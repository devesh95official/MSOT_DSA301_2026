...
void solve() {
    int n; cin >> n;
    vector<int> cnt(n, 0);         // cnt[r] = prefixes seen so far with remainder r
    cnt[0] = 1;                    // empty prefix
    int pref = 0, ans = 0;
    for (int i = 0; i < n; i++) {
        int v; cin >> v;
        pref = ((pref + v) % n + n) % n;   // keep the remainder in [0, n) even for negatives
        ans += cnt[pref];                  // same remainder earlier -> divisible subarray
        cnt[pref]++;
    }
    cout << ans << '\n';
}
