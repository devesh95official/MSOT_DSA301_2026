...
void solve() {
    int n; cin >> n;
    int pref = 0, minPref = 0, best = LLONG_MIN;   // minPref = min over P[0..r-1]
    for (int r = 1; r <= n; r++) {
        int x; cin >> x;
        pref += x;                                 // pref = P[r]
        best = max(best, pref - minPref);          // best subarray ending at r
        minPref = min(minPref, pref);              // now P[r] may be used later
    }
    cout << best << '\n';
}
