...
void solve() {
    int n, x; cin >> n >> x;
    multiset<int> ms;
    for (int i = 0; i < n; i++) { int v; cin >> v; ms.insert(v); }

    int gondolas = 0;
    while (!ms.empty()) {
        // take the lightest remaining child...
        int light = *ms.begin();
        ms.erase(ms.begin());
        // ...and pair him with the HEAVIEST child that still fits
        auto it = ms.upper_bound(x - light);
        if (it != ms.begin()) ms.erase(prev(it));
        gondolas++;
    }
    cout << gondolas << '\n';
}
