...
void solve() {
    ...
    // sum(l+1..r) = P[r] - P[l] with  A <= r - l <= B   ->   l in [r - B, r - A]
    multiset<int> window;                  // the P[l] values that are currently allowed
    int best = LLONG_MIN;
    for (int r = A; r <= n; r++) {
        window.insert(P[r - A]);            // l = r - A just became allowed
        if (r - B - 1 >= 0) window.erase(window.find(P[r - B - 1]));   // l = r-B-1 fell out
        best = max(best, P[r] - *window.begin());   // subtract the SMALLEST allowed prefix
    }
    cout << best << '\n';
}
