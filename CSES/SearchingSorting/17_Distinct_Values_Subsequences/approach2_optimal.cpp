...
void solve() {
    ...
    vector<int> a(n);
    for (auto &v : a) cin >> v;
    sort(a.begin(), a.end());              // equal values form one block

    int ways = 1;
    for (int i = 0; i < n; ) {
        int j = i;
        while (j < n && a[j] == a[i]) j++; // block [i, j) has f = j - i copies
        ways = ways * (j - i + 1) % mod;   // skip the value, or pick one of its f copies
        i = j;
    }
