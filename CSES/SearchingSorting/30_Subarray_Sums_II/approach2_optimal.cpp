...
void solve() {
    int n, x; cin >> n >> x;
    map<int,int> freq;              // freq[s] = how many prefixes seen so far have sum s
    freq[0] = 1;                    // the empty prefix P[0] = 0
    int pref = 0, cnt = 0;
    for (int i = 0; i < n; i++) {
        int v; cin >> v;
        pref += v;                  // pref = P[r]
        cnt += freq[pref - x];      // every earlier P[l] = P[r] - x gives one subarray
        freq[pref]++;
    }
    cout << cnt << '\n';
}
