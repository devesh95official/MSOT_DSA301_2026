...
void solve() {
    ...
    seen[0] = false;                          // value 0 doesn't exist -> "1" always opens a round
    int rounds = 0;
    for (int i = 0; i < n; i++) {
        int x; cin >> x;
        if (!seen[x - 1]) rounds++;           // x-1 hasn't appeared yet, so x cannot be picked
        seen[x] = true;                       // in the same pass as x-1 -> x starts a new round
    }
    cout << rounds << '\n';
}
