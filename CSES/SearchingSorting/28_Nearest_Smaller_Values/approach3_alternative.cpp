...
// "Pointer jumping": ans[i] starts at i-1 and follows the chain of answers.
// If a[j] >= a[i], nothing between ans[j] and j can help, so jump to ans[j].
void solve() {
    int n; cin >> n;
    vector<int> a(n + 1), ans(n + 1, 0);
    a[0] = LLONG_MIN;                 // sentinel at position 0: smaller than everything
    for (int i = 1; i <= n; i++) cin >> a[i];

    for (int i = 1; i <= n; i++) {
        int j = i - 1;
        while (a[j] >= a[i]) j = ans[j];
        ans[i] = j;
        cout << j << " \n"[i == n];
    }
}
